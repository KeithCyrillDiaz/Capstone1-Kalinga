...
const ResetPasswordCode = ({route}) => {
    ...
    const codeValidation = async (code) => {
        ...
        const Valid = await axios.get(`${BASED_URL}/kalinga/checkPassCode/${code}`)
        if(Valid.data.messages.code !== 0){
            Alert.alert('Invalid Code', 'The verification code you entered is incorrect. Please check and try again.');
            return
        } else {
            ...
            navigation.dispatch(
                CommonActions.reset({
                  ...
                  routes: [{ name: "SetPassword", params: {email: Email, Applicant_ID: null, userType: null }}], // Replace 'Login' with the name of your login screen
                })
              );
        }
    }
