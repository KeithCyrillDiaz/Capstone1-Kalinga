...
const donorAppointmentConfirmation = () => {
  ...
  useEffect(() => {
    const fetchAppointmentData = async () => {
      try {
        ...
        const response = await axios.get(`${WebHost}/kalinga/getAppointmentsByDonorID/${AppointmentDonorID}`);
  ...
  const handleApproved = async () => {
    ...
    try {
      ...
      await axios.put(`${WebHost}/kalinga/updateDonationStatus/${AppointmentDonorID}`, {
  ...
  const handleDecline = () => {
    ...
    } else if (appointmentData && appointmentData.DonationStatus === 'Ongoing') {
  ...
  const handleDeclineConfirm = async () => {
    try {
      ...
      const response = await axios.put(`${WebHost}/kalinga/updateDonationStatus/${AppointmentDonorID}`, {
  ...
  return (
    <section className="w-full min-h-screen bg-neutral-variant p-8 relative">
      ...
      <div className="mt-8">
        <input
          ...
          id="fullName"
          name="fullName"
          value={`Full Name: ${appointmentData ? appointmentData.fullName : ''}`}
      ...
      <div className="mt-4">
        <input
          ...
          id="phoneNumber"
          name="phoneNumber"
          value={`Phone Number: ${appointmentData ? appointmentData.phoneNumber : ''}`}
          onChange={handleChange}
      ...
      <div className="mt-4">
        <input
          ...
          id="emailAddress"
          name="emailAddress"
          value={`Email Address: ${appointmentData ? appointmentData.emailAddress : ''}`}
          onChange={handleChange}
          className="w-full px-4 py-2 h-14 border border-pink-500 rounded-lg focus:outline-none focus:border-pink-500 text-pink-500"
      ...
      <div className="mt-4">
        <input
          ...
          id="homeAddress"
          name="homeAddress"
          value={`Home Address: ${appointmentData ? appointmentData.homeAddress : ''}`}
          onChange={handleChange}
      ...
      <div className="mt-4">
        <input
          ...
          value={`Medical Condition: ${appointmentData ? appointmentData.medicalCondition : ''}`}
          onChange={handleChange}
          className="w-full px-4 py-2  h-14 border border-pink-500 rounded-lg focus:outline-none focus:border-pink-500 text-pink-500"
      ...
      <div className="mt-4">
        <input
          ...
          id="amountDonated"
          name="amountDonated"
          value={`Milk Amount: ${appointmentData ? appointmentData.milkAmount : ''}`}
          onChange={handleChange}
          className="w-full px-4 py-2 h-14 border border-pink-500 rounded-lg focus:outline-none focus:border-pink-500 text-pink-500"
      ...
      <div className="flex mt-4">
        ...
        <label htmlFor="appointmentDate" className="block text-pink-500 font-bold mb-2 text-pink-500">
            Date
        </label>
        ...
        <div className="w-1/2 relative">
            <label htmlFor="appointmentTime" className="block text-pink-500 font-bold mb-2 text-pink-500">
                Time
            </label>
            <div className="relative">
                <input
      ...
      <div className="absolute bottom-0 right-0 mb-8 mr-8 flex flex-col">
        <button onClick={handleApproved} className="bg-pink-500 text-white px-4 py-2 rounded-full mb-4">
          Approved
        </button>
        <button onClick={handleDecline} className="bg-white text-pink-500 px-4 py-2 rounded-full border border-pink-500">
          Decline
        </button>
      </div>
    )}
    ...
    {appointmentData && appointmentData.DonationStatus === 'Ongoing' && (
      <div className="absolute bottom-0 right-0 mb-8 mr-8 flex flex-col">
      <button onClick={handleComplete} className="bg-pink-500 text-white px-4 py-2 rounded-full mb-4">
        Complete
      </button>
      </div>
    )}
