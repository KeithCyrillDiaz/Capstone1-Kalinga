...
const DonorAppointments = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const response = await axios.get(`${WebHost}/kalinga/getAppointmentByUserType/Donor`);
  ...
  const filteredAppointments = appointments.filter((appointment) =>
    appointment.fullName.toLowerCase().includes(searchQuery.trim().toLowerCase())
  );
      ...
      {currentAppointments.map((appointment) => (
        ...
        <div key={appointment._id} className="flex mb-4 mt-12">
          <div className="w-1/4 pr-8 text-xl">
            <div className="text-pink-500 p-2 text-center rounded-lg font-bold">{appointment.fullName}</div>
          </div>
          <div className="w-1/4 pl-8 pr-8 text-xl">
            <div className="text-pink-500 p-2 text-center rounded-lg font-bold">{appointment.DonationStatus}</div>
          </div>
          <div className="w-1/4 pl-8 ml-36">
          <Link to={`/admin/donorAppointmentConfirmation/${appointment.AppointmentDonorID}`}>
