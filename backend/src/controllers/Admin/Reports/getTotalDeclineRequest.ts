...
export const getTotalDeclineRequestPerMonth = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const declineRequest = await RequestModel.find({
      RequestStatus: 'Decline',
    });

    const filteredAppointments = declineRequest.filter(request => {
    ...
    const totalDeclineRequest = filteredAppointments.length;
