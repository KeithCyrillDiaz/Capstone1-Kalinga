...
const Bugs = () => {
  ...
  return (
    <section className="w-full min-h-screen bg-neutral-variant p-8">
      ...
      {activeTab === "report" && (
        <div>
          <h2 className="text-2xl font-semibold text-red-500 mb-4">Report Bugs</h2>
              {bugReports && bugReports.length > 0 ? (
                  bugReports.map((bug) => (
                    <div key={bug._id} className="bg-white rounded-lg p-4 mb-4 flex items-center">
                      <div className="mr-4">
                        <svg
                      ...
                      <div className="flex-grow">
                        <div>
                          <h3 className="text-lg font-semibold text-red-500 mb-2">
                            {bug.DonorOwnerID ? bug.DonorOwnerID.fullName || "Unknown" : bug.RequestorOwnerID ? bug.RequestorOwnerID.fullName || "Unknown" : "Unknown"}
                          </h3>
                          <p className="text-lg text-gray-600">
                            {bug.DonorOwnerID ? bug.DonorOwnerID.email || "Unknown" : bug.RequestorOwnerID ? bug.RequestorOwnerID.email || "Unknown" : "Unknown"}
                          </p>
     ...
     {activeTab === "resolved" && (
        <div>
          <h2 className="text-2xl font-semibold text-red-500 mb-4">Resolved Bugs</h2>
          {bugReports && bugReports.length > 0 ? (
            bugReports.map((bug) => (
              <div key={bug._id} className="bg-white rounded-lg p-4 mb-4 flex items-center">
                <div className="mr-4">
                ...
                <div className="flex-grow">
                  <div>
                  <h3 className="text-lg font-semibold text-red-500 mb-2">{bug.DonorOwnerID ? bug.DonorOwnerID.fullName || "Unknown" : bug.RequestorOwnerID ? bug.RequestorOwnerID.fullName || "Unknown" : "Unknown"}</h3>
                      <p className="text-lg text-gray-600">{bug.DonorOwnerID ? bug.DonorOwnerID.email || "Unknown" : bug.RequestorOwnerID ? bug.RequestorOwnerID.email || "Unknown" : "Unknown"}</p>
                    <p className="text-lg text-gray-600">{`Created At: ${bug.createdAt}`}</p>
                  </div>
                </div>
                <div className="mr-8">
